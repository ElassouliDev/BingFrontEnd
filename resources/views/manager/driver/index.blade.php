                    ...
                    <div class="kt-portlet__head-toolbar">
                        <div class="kt-portlet__head-wrapper">
                            <div class="kt-portlet__head-actions">
                                <a href="{{ route('manager.driver.create') }}" class="btn btn-danger btn-elevate btn-icon-sm">
                    ...
                    <form class="kt-form kt-form--fit kt-margin-b-20">
                        <div class="row kt-margin-b-20">
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <input type="text" name="name" id="name" class="form-control kt-input" placeholder="{{t('Driver Name')}}">
                            </div>
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                <label>{{ t('Status') }}:</label>
                                <select class="form-control" name="status" id="status">
                                    <option selected value="">{{t('Select Status')}}</option>
                                    <option value="{{\App\Models\User::ACTIVE}}" >{{t('Active')}}</option>
                                    <option value="{{\App\Models\User::NOT_ACTIVE}}" >{{t('Not Active')}}</option>
                                    <option value="{{\App\Models\User::NEW_ACCOUNT}}" >{{t('New Account')}}</option>
                                </select>
                            </div>
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
    ...
    <script>
        $(document).ready(function(){
            $(document).on('click','.deleteRecord',(function(){
                var id = $(this).data("id");
                var url = '{{ route("manager.driver.destroy", ":id") }}';
            ...
            $(function() {
                $('#users-table').DataTable({
                    ...
                    searching: false,
                    ajax: {
                        url : '{{ route('manager.driver.index') }}',
                        data: function (d) {
                            d.name = $("#name").val();
                            d.status = $("#status").val();
                            d.source = $("#source").val();
                    ...
                    columns: [
                        ...
                        {data: 'driver_type_name', name: 'driver_type_name'},
