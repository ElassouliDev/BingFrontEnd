...
@section('content')
    @push('breadcrumb')
        <li class="breadcrumb-item">
            <a href="{{ route('manager.driver.index') }}">{{t('Drivers')}}</a>
        </li>
        <li class="breadcrumb-item">
        ...
        <div class="col-xl-12">
            <div class="kt-portlet">
                ...
                <form enctype="multipart/form-data" id="form_information" class="kt-form kt-form--label-right"
                      action="{{route('manager.driver.store') }}"
                      method="post">
                    {{ csrf_field() }}
                    @if(isset($driver))
...
{{--                                </div>--}}
                                ...
                                <div class="form-group row" id="driver_type">
                                    <label class="col-xl-3 col-lg-3 col-form-label">{{ t('Driver Type') }}</label>
                                    <div class="col-lg-9 col-xl-6">
                                        <select class="form-control driver_type" name="driver_type">
                                            <option value="" selected disabled>{{t('Select Driver Type')}}</option>
                                            <option
                                                value="{{\App\Models\User::FOLLOWED_TO_RESTAURANT_DRIVER}}" {{isset($driver) && $driver->driver_type == \App\Models\User::FOLLOWED_TO_RESTAURANT_DRIVER? 'selected' : ''}} >{{t('Followed to restaurant')}}</option>
                                            <option
                                                value="{{\App\Models\User::FREELANCER_DRIVER}}" {{isset($driver) && $driver->driver_type == \App\Models\User::FREELANCER_DRIVER? 'selected' :''}}>{{t('Freelancer driver')}}</option>
                                        </select>

                                    </div>
                                </div>
                                <div class="form-group row" id="iban" style="display: {{isset($driver) && $driver->driver_type == \App\Models\User::FREELANCER_DRIVER?'' : 'none'}}">
                                    <label class="col-xl-3 col-lg-3 col-form-label">{{ t('IBAN') }}</label>
                                    <div class="col-lg-9 col-xl-6">
                                        <input class="form-control" name="i_ban" type="text"
                                               value="{{ isset($driver) ? $driver->i_ban :old('i_ban') }}">
                                    </div>
                                </div>
                                <div class="form-group row" id="restaurant" style="display: {{isset($driver) && $driver->driver_type == \App\Models\User::FOLLOWED_TO_RESTAURANT_DRIVER?'' : 'none'}}">
                                    <label class="col-xl-3 col-lg-3 col-form-label">{{ t('Restaurant') }}</label>
                                    <div class="col-lg-9 col-xl-6">
                                        <select class="form-control restaurants" name="merchant_id">
                                            ...
                                            @foreach($restaurants as $restaurant)
                                                <option
                                                    value="{{$restaurant->id}}" {{isset($driver) && $driver->merchant_id == $restaurant->id ? 'selected':''}}>{{$restaurant->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row" id="branch"
                                     style="display: {{isset($driver) && $driver->driver_type == \App\Models\User::FOLLOWED_TO_RESTAURANT_DRIVER?'' : 'none'}}">
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $('.driver_type').change(function () {
            var id = $(this).val();
            if (id == {{\App\Models\User::FOLLOWED_TO_RESTAURANT_DRIVER}}) {
                $('#restaurant').show();
                $('#branch').show();
                $('#iban').hide();
            } else {
