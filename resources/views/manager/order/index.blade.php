...
@section('content')
                        ...
                        <div class="row kt-margin-b-20">
                            ...
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <select class="form-control" name="status" id="status">
                                    <option {{!request()->status? 'selected' : ''}}  value="">{{t('Select Status')}}</option>
                                    <option  {{!is_null(request()->status) && request()->status == \App\Models\Order::PENDING? 'selected' : ''}}  value="{{\App\Models\Order::PENDING}}" >{{t('Pending')}}</option>
                                    <option {{!is_null(request()->status) && request()->status == \App\Models\Order::ACCEPTED? 'selected' : ''}}value="{{\App\Models\Order::ACCEPTED}}" >{{t('Accepted')}}</option>
                                    <option {{!is_null(request()->status) && request()->status == \App\Models\Order::ON_PROGRESS? 'selected' : ''}} value="{{\App\Models\Order::ON_PROGRESS}}" >{{t('On Progress')}}</option>
                                    <option {{!is_null(request()->status) && request()->status == \App\Models\Order::READY? 'selected' : ''}} value="{{\App\Models\Order::READY}}" >{{t('Ready')}}</option>
                                    <option {{!is_null(request()->status) && request()->status == \App\Models\Order::ON_WAY? 'selected' : ''}} value="{{\App\Models\Order::ON_WAY}}" >{{t('On Way')}}</option>
                                    <option {{!is_null(request()->status) && request()->status == \App\Models\Order::COMPLETED? 'selected' : ''}} value="{{\App\Models\Order::COMPLETED}}" >{{t('Completed')}}</option>
                                    <option {{!is_null(request()->status) && request()->status == \App\Models\Order::CANCELED? 'selected' : ''}} value="{{\App\Models\Order::CANCELED}}" >{{t('Canceled')}}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row kt-margin-b-20">
                            ...
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <div class="input-group" id="">
                                    <input type="text" class="form-control" name="price_start" id="price_start" placeholder="{{t('From')}}">
                                    <div class="input-group-append">
                                        <span class="input-group-text"><i class="la la-ellipsis-h"></i></span>
                                    </div>
                                    <input type="text" name="price_end" class="form-control" id="price_end" placeholder="{{t('To')}}">
                                </div>
                            </div>
                            ...
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <div class="input-group date">
                                    <input type="text" class="form-control date_time" name="date_start" readonly="" placeholder="{{t('Select date')}}" id="">
                                    <div class="input-group-append">
                            ...
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <div class="input-group date">
                                    <input type="text" class="form-control date_time" name="date_end" readonly="" placeholder="{{t('Select date')}}" id="">
                                    <div class="input-group-append">
                                        <span class="input-group-text"><i class="la la-calendar-o glyphicon-th"></i></span>
                                    </div>
                                </div>
                            ...
                            <div class="col-lg-4 kt-margin-b-10-tablet-and-mobile">
                                <label>{{ t('Total') }}:</label>
                                <br />
                                <h3 class="w-100 text-center">{{t('Total Orders')}} : <span class="text-warning">{{$total}}</span></h3>
                            </div>
                        </div>
                    </form>
    ...
    <div class="modal fade" id="statusModel" tabindex="-1" role="dialog" aria-labelledby="statusModel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form method="post" action="" id="status_form">
                    ...
                    <div class="modal-body">
                        <select class="form-control" name="status">
                            ...
                            <option id="status_{{\App\Models\Order::ACCEPTED}}" value="{{\App\Models\Order::ACCEPTED}}">{{t('Accepted')}}</option>
                            <option id="status_{{\App\Models\Order::ON_PROGRESS}}" value="{{\App\Models\Order::ON_PROGRESS}}">{{t('On Progress')}}</option>
                            <option id="status_{{\App\Models\Order::READY}}" value="{{\App\Models\Order::READY}}">{{t('Ready')}}</option>
{{--                            <option id="status_{{\App\Models\Order::COMPLETED}}" value="{{\App\Models\Order::COMPLETED}}">{{t('Completed')}}</option>--}}
                            <option id="status_{{\App\Models\Order::CANCELED}}" value="{{\App\Models\Order::CANCELED}}">{{t('Canceled')}}</option>
                        </select>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{w('Cancel')}}</button>
