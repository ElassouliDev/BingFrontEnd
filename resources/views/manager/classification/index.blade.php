                    ...
                    <div class="kt-portlet__head-toolbar">
                        <div class="kt-portlet__head-wrapper">
                            <div class="kt-portlet__head-actions">
                                <a href="{{ route('manager.classification.create') }}"
                        ...
                        <div class="row kt-margin-b-20">
                            ...
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <select class="form-control" name="merchant" id="merchant">
                                    <option selected value="">{{t('Select Merchant')}}</option>
                                    @foreach($merchants as $index=>$item)
                                        <option
                                            value="{{$item->id}}" {{request()->merchant == $item->id? 'selected' : ''}}>{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-2 kt-margin-b-10-tablet-and-mobile">
                                ...
                                <select class="form-control" name="branch" id="branch">
                                    ...
                                    @if(isset($branches))
                                        @foreach($branches as $index=>$item)
                                            <option
                                                value="{{$item->id}}" {{request()->branch == $item->id? 'selected' : ''}}>{{$item->name}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
    ...
    <script>
        $(document).ready(function () {
            $(document).on('click', '.deleteRecord', (function () {
                var id = $(this).data("id");
                var url = '{{ route("manager.classification.destroy", ":id") }}';
