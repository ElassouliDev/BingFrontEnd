...
@section('content')
    @push('breadcrumb')
        <li class="breadcrumb-item">
            <a href="{{ route('manager.classification.index') }}">{{t('Classifications')}}</a>
        </li>
        <li class="breadcrumb-item">
        ...
        <div class="col-xl-12">
            <div class="kt-portlet">
                ...
                <form enctype="multipart/form-data" id="form_information" class="kt-form kt-form--label-right"
                      action="{{route('manager.classification.store') }}" method="post">
                    {{ csrf_field() }}
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-9 col-xl-6">
                                        <select class="form-control restaurants" name="merchant_id">
                                            <option value="" selected disabled>{{t('Select Restaurant')}}</option>
                                            @foreach($restaurants as $restaurant)
                                                <option
                                                    value="{{$restaurant->id}}" {{isset($classification) && $classification->merchant_id == $restaurant->id ? 'selected':''}}>{{$restaurant->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
