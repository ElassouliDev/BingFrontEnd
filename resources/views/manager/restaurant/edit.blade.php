...
@section('content')
    @push('breadcrumb')
        ...
        <li class="breadcrumb-item">
            {{ isset($merchant) ? t('Edit Restaurant') : t('Add Restaurant') }}
        </li>
    @endpush
    @php
        $name = isset($merchant) ? $merchant->getTranslations()['name'] : null;
    @endphp
            ...
            <div class="kt-portlet">
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">{{ isset($merchant) ? t('Edit Restaurant') : t('Add Restaurant') }}</h3>
                    </div>
                </div>

                    <form enctype="multipart/form-data" id="form_information" class="kt-form kt-form--label-right"
                          action="{{route('manager.restaurant.store') }}"
                          method="post">
                        {{ csrf_field() }}
                        @if(isset($merchant))
...
{{--                                </div>--}}
                                ...
                                @foreach(config('translatable.locales') as $local)
                                    <div class="form-group row">
                                        ...
                                        <div class="col-lg-9 col-xl-6">
                                            <input class="form-control" name="name[{{$local}}]" type="text" value="{{ isset($name) ? $name[$local] : old("name[$local]") }}">
                                        </div>
                                    </div>
                                @endforeach
                                <div class="form-group row">
