...
@section('content')
    @push('breadcrumb')
        ...
        <li class="breadcrumb-item">
            <a href="{{ route('branch.driver.index') }}">{{ t('Drivers') }}</a>
        </li>
        <li class="breadcrumb-item">
            {{ t('Show Driver') }}
        </li>
                            ...
                            <div class="kt-widget__content">
                                ...
                                <div class="kt-widget__info">
                                    <div class="kt-widget__desc">
                                        ...
                                        <br>
                                        <label href="#" class="mr-2">{{ t('Gender') .' : '. $user->gender }}</label>
                                    </div>
                                </div>
                            </div>
                            <div class="kt-widget__media kt-hidden-">
    ...
    <div class="modal fade" id="notificationModel" tabindex="-1" role="dialog" aria-labelledby="notificationModel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('branch.user_notification')}}" id="form_information">
    ...
    <div class="modal fade" id="walletModel" tabindex="-1" role="dialog" aria-labelledby="walletModel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('branch.wallet.store', $user->id)}}" id="wallet_form">
                    <input type="hidden" name="user_id" value="{{$user->id}}">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <div class="container">
    ...
    <script>
        $(document).ready(function(){
            $(document).on('click','.deleteRecord',(function(){
                var id = $(this).data("id");
                var url = '{{ route("branch.wallet.destroy", ":id") }}';
                url = url.replace(':id', id );
                $('#delete_form').attr('action',url);
            }));
            $(document).on('click','.notifyDeleteRecord',(function(){
                var id = $(this).data("id");
                var url = '{{ route("branch.notification.destroy", ":id") }}';
                url = url.replace(':id', id );
            ...
            $(function() {
                $('#wallet-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('branch.wallet.index', $user->id) }}',
                    },
                    columns: [
            ...
            $(function() {
                var table = $('#notification-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('branch.user.notifications') }}',
                        data: function (d) {
                            d.user = "{{$user->id}}"
                        }
                    },
            ...
            $(function() {
                var table = $('#orders-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('branch.deliveries_dashboard') }}',
                        data: function (d) {
                            d.driver_id = "{{$user->id}}"
                        }
                    },
            ...
            $(function() {
                $('#rates-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('branch.client_driver_rate.index') }}',
                        data: function (d) {
                            d.driver = "{{$user->id}}";
                            d.search = $("#search").val();
                        }
