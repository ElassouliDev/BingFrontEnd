...
@section('content')
    @push('breadcrumb')
        <li class="breadcrumb-item">
            <a href="{{ route('restaurant.meal.index') }}">{{t('Meals')}}</a>
        </li>
        <li class="breadcrumb-item">
        ...
        <div class="col-xl-12">
            <div class="kt-portlet">
                ...
                <form enctype="multipart/form-data" id="form_information" class="kt-form kt-form--label-right"
                      action="{{route('restaurant.meal.store') }}" method="post">
                    {{ csrf_field() }}
                    @if(isset($meal))
    ...
    <script>
        $(document).ready(function () {
            $('.restaurants').change(function () {
                var id = $(this).val();
                var url = '{{ route("restaurant.branches") }}';
                $.ajax({
                    type: "get",
                    url: url,
                    data: {
            ...
            $('.branches').change(function () {
                ...
                var url = '{{ route("restaurant.classifications") }}';
                $.ajax({
                    type: "get",
                    url: url,
                    data: {
            ...
            $(wrapper).on('click', '.removed_button', function (e) {
                ...
                {{--var url = '{{route('restaurant.price.delete', ':id')}}';--}}
            ...
            $(addon_wrapper).on('click', '.removed_addon', function (e) {
                ...
                {{--let csrf = $('meta[name="csrf-token"]').attr('content');--}}
                {{--var id = $(this).attr('data-id');--}}
                {{--var url = '{{route('restaurant.addon.delete', ':id')}}';--}}
                {{--url = url.replace(':id', id);--}}
                {{--$.ajax({--}}
                {{--    url: url,--}}
                {{--    type: 'POST',--}}
