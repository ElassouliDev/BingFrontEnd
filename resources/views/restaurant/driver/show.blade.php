    ...
    <div class="modal fade" id="notificationModel" tabindex="-1" role="dialog" aria-labelledby="notificationModel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('restaurant.user_notification')}}" id="form_information">
    ...
    <div class="modal fade" id="walletModel" tabindex="-1" role="dialog" aria-labelledby="walletModel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('restaurant.wallet.store', $user->id)}}" id="wallet_form">
                    <input type="hidden" name="user_id" value="{{$user->id}}">
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <div class="container">
    ...
    <script>
        $(document).ready(function(){
            $(document).on('click','.deleteRecord',(function(){
                var id = $(this).data("id");
                var url = '{{ route("restaurant.wallet.destroy", ":id") }}';
                url = url.replace(':id', id );
                $('#delete_form').attr('action',url);
            }));
            $(document).on('click','.notifyDeleteRecord',(function(){
                var id = $(this).data("id");
                var url = '{{ route("restaurant.notification.destroy", ":id") }}';
                url = url.replace(':id', id );
            ...
            $(function() {
                $('#wallet-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('restaurant.wallet.index', $user->id) }}',
                    },
                    columns: [
            ...
            $(function() {
                var table = $('#notification-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('restaurant.user.notifications') }}',
                        data: function (d) {
                            d.user = "{{$user->id}}"
                        }
                    },
            ...
            $(function() {
                var table = $('#orders-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('restaurant.deliveries_dashboard') }}',
                        data: function (d) {
                            d.driver_id = "{{$user->id}}"
                        }
                    },
