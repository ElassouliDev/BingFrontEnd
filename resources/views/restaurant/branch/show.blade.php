                            ...
                            <div class="kt-widget__content">
                                <div class="kt-widget__head">
                                    ...
                                    <div class="kt-widget__action">
                                        ...
                                        <a href="{{ route('restaurant.go_branch', $branch->id) }}" class="btn btn-brand btn-sm btn-upper">{{t('Go To Branch')}}</a>
                                        <a href="{{ route('restaurant.change_drivers_code', $branch->id) }}"
    ...
    <div class="modal fade" id="notificationModel" tabindex="-1" role="dialog" aria-labelledby="notificationModel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{route('restaurant.branch_notification')}}" id="form_information">
...
@section('script')
    ...
    <script>
        $(document).ready(function(){
            ...
            {{--        ],--}}
            {{--        ajax: {--}}
            {{--            url : '{{ route('restaurant.branch.notifications') }}',--}}
            {{--            data: function (d) {--}}
            {{--                d.user = "{{$branch->id}}"--}}
            {{--            }--}}
            {{--        },--}}
            ...
            $(function() {
                $('#classifications-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('restaurant.classification.index') }}',
                        data: function (d) {
                            d.branch = "{{$branch->id}}";
                            d.search = $("#search").val();
                        }
            ...
            $(function() {
                $('#rates-table').DataTable({
                    ...
                    ajax: {
                        url : '{{ route('restaurant.client_order_rate.index') }}',
                        data: function (d) {
                            d.branch = "{{$branch->id}}";
                            d.search = $("#search").val();
                        }
