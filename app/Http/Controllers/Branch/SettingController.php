...
class SettingController extends Controller
{
    ...
    {
        $order = Order::currentBranch(user('branch')->id);
...
//        $total_orders = $order->where('status', '<>', Order::PENDING)->sum('total_cost');
        ...
        return view('branch.home', compact('total_orders', 'total_payments', 'orders',
    ...
    public function view_profile()
    {
        ...
        return view('branch.profile', compact('title', 'merchant', 'validator', 'merchant_types', 'banks', 'categories', 'days'));

        return view('branch.profile', compact('title'));
    }
...
//        dd(checkRequestIsWorkingOrNot());
        ...
        if ($request->hasFile('image')) {
            $store->image = $this->uploadImage($request->file('image'), 'branchs');
        }
        if ($request->hasFile('id_file')) {
            $store->id_file = $this->uploadImage($request->file('id_file'), 'branchs');
        }
        if ($request->hasFile('comm_registration_file')) {
            $store->comm_registration_file = $this->uploadImage($request->file('comm_registration_file'), 'branchs');
        }
        $store->save();
