...
class ItemController extends Controller
{
    ...
    public function index()
    {
        ...
        $classifications = Classification::currentBranch(user('branch')->id)->get();
        return view('branch.item.index', compact('title','classifications'));
    }
    ...
    public function create()
    {
        ...
        return view('branch.item.edit', compact('title', 'validator', 'options_categories'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('branch.item.edit', compact('title', 'validator',
            'meal', 'classifications', 'options_categories', 'option_category'));
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) $store->image = $this->uploadImage($request->file('image'), 'meals');
        $store->save();
        ...
        foreach ($db_prices as $index => $db_price) if (!in_array($db_price, $newArr_prices)) ItemPrice::where('id', $db_price)->delete();
        foreach ($db_addons as $index => $db_addon) if (!in_array($db_addon, $newArr_addons)) ItemAddon::where('id', $db_addon)->delete();
        ...
        if (isset($request->item_id)) {
            return redirect()->route('branch.meal.index')->with('m-class', 'success')->with('message', t('Successfully Updated'));
        } else {
            return redirect()->route('branch.meal.index')->with('m-class', 'success')->with('message', t('Successfully Created'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('branch.meal.index')->with('message', t('Successfully Updated'))->with('m-class', 'success');
    }
