...
//        $this->validationRules["password"] = password_rules(true, 6);
    ...
    public function index()
    {
        ...
        return view('branch.driver.index', compact('title'));
    }
    ...
    public function create()
    {
        ...
        return view('branch.driver.edit', compact('title', 'validator', 'cities', 'banks', 'nationalities', 'transporters'));
    }
...
//        $this->validationRules["password"] = 'nullable|min:6';
        ...
        $nationalities = Nationality::get();
        $transporters = TransporterType::get();
        return view('branch.driver.edit', compact('title', 'validator', 'cities', 'banks', 'nationalities', 'transporters', 'driver'));
    }
...
//        if (isset($request->password)) $store->password = Hash::make($request->password);
        ...
        if (isset($request->driver_id)) {
            return redirect()->route('branch.driver.index')->with('m-class', 'success')->with('message', t('Successfully Updated'));
        } else {
            return redirect()->route('branch.driver.index')->with('m-class', 'success')->with('message', t('Successfully Created'));
        }
    }
    ...
    public function show($id)
    {
        ...
        return view('branch.driver.show', compact('user', 'title', 'data', 'notify_validator', 'validator'));
    }


    public function destroy($id)
    {
        $item = $this->_model->driver()->currentBranch(\user('branch')->id)->findOrFail($id);
