...
class ClientDriverRateController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            $rates = $this->_model->when($driver, function ($query1) use ($driver) {
                $query1->whereHas('driver', function ($query) use ($driver) {
                    $query->where('id', $driver)->whereHas('branch', function ($q) {
        ...
        return view('restaurant.rate.client_driver_rate', compact('title', 'branches'));
    }
    ...
    public function destroy($id)
    {
        ...
        $rate->delete();
        return redirect()->route('restaurant.client_driver_rate.index')->with('m-class', 'success')->with('message', t('Successfully Deleted'));
    }
}
