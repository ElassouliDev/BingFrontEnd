...
//            dd(checkRequestIsWorkingOrNot());
            return DataTables::make($orders)
                ...
                ->addColumn('show_action', function ($order) {
                    return '<a href="' . route('restaurant.order.show', $order->id) . '" class="btn btn-icon btn-danger "><i class="la la-eye"></i></a> ';
                })
                ->make();
        }
        $total = $orders->sum('total_cost');
        $branches = Branch::currentMerchant(user('merchant')->id)->get();
        return view('restaurant.order.index', compact('title', 'branches', 'total'));
    }
    ...
    public function show($id)
    {
        ...
        return view('restaurant.order.show', compact('order', 'title'));
    }
...
//        TODO send notification to user and drivers
    ...
    public function destroy($id)
    {
        $order = Order::query()->findOrFail($id);
//        $order->delete();
        return redirect()->route('restaurant.order.index')->with('message', t('Successfully Deleted'))->with('m-class', 'success');
    }
