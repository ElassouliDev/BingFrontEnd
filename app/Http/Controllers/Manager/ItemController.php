...
//        $this->validationRules["branch_id"] = 'required|exists:users,id';
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $items = $this->_model->when($name, function ($query) use ($name) {
                ...
                $query->where('merchant_id', $restaurant);
        ...
        return view('manager.item.index', compact('title', 'restaurants'));
    }
    ...
    public function create()
    {
        ...
        return view('manager.item.edit', compact('title', 'validator', 'restaurants', 'options_categories'));
    }
    ...
    public function edit($id)
    {
        ...
        $validator = JsValidator::make($this->validationRules, $this->validationMessages);
        $restaurants = Merchant::get();
//        $branchesIds = $meal->branches->pluck('id')->all();
        ...
        return view('manager.item.edit', compact('title', 'validator',
    ...
    public function store(Request $request)
    {
        ...
        $store->merchant_id = $request->restaurant_id;
        ...
        $min_price = ItemPrice::query()->whereHas('item', function ($query) use ($store, $request) {
            $query->whereHas('branches', function ($query) use ($store, $request) {
                $query->where('branches.id', (array)$request->branch_id);
            });
        })->min('price');
        $store->branch()->update([
            'min_price' => $min_price,
        ...
        if (isset($request->item_id)) {
            return redirect()->route('manager.meal.index')->with('m-class', 'success')->with('message', t('Successfully Updated'));
        } else {
            return redirect()->route('manager.meal.index')->with('m-class', 'success')->with('message', t('Successfully Created'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('manager.meal.index')->with('message', t('Successfully Updated'))->with('m-class', 'success');
    }
