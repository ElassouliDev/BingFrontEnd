...
class DriverClientRateController extends Controller
{
    private $_model;

    public function __construct(DriverClientRate $driverClientRate)
    {
        parent::__construct();
        $this->_model = $driverClientRate;
        $this->middleware('permission:Ratings', ['only' => ['destroy']]);
    }

    public function index(Request $request)
    {
        $title = t('Client Rates');
        if (request()->ajax()) {
        ...
        return view('manager.rate.driver_client_rate', compact('title'));
    }
