...
//        $this->validationRules["category_id"] = 'required|exists:categories,id';
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $branches = $this->_model
                ...
                ->when($merchant_type, function ($query) use ($merchant_type) {
                    $query->whereHas('merchant', function ($qu) use ($merchant_type) {
                        $qu->where('merchant_type_id', $merchant_type);
                    });
                })
        ...
        return view('manager.branch.index', compact('title', 'merchant_types', 'merchants'));
    }
    ...
    public function change_drivers_code($id)
    {
        ...
        return redirect()->route('manager.branch.show', $id)->with('m-class', 'success')->with('message', t('Successfully Updated'));
    }
    ...
    public function create()
    {
        ...
        return view('manager.branch.edit', compact('title', 'validator', 'restaurants', 'days'));
    }
...
//        dd($branch,$branchDays,$branchDaysId);
        ...
        return view('manager.branch.edit', compact('title', 'branch', 'validator', 'restaurants', 'days'));
    }
...
//        dd($this->validationRules);
        ...
        if ($request->hasFile('image')) $store->image = $this->uploadImage($request->file('image'), 'branches');
        if ($request->hasFile('cover')) $store->cover = $this->uploadImage($request->file('cover'), 'branches');
...
//        dd(checkRequestIsWorkingOrNot());
        ...
        if (isset($request->branch_id)) {
            return redirect()->route('manager.branch.index')->with('m-class', 'success')->with('message', t('Successfully Updated'));
        } else {
            return redirect()->route('manager.branch.index')->with('m-class', 'success')->with('message', t('Successfully Created'));
    ...
    public function show($id)
    {
        ...
        $items = Item::whereHas('branches', function ($query) use ($branch) {
        ...
        return view('manager.branch.show', compact('branch', 'title', 'data', 'validator'));
    }
    ...
    public function destroy($id)
    {
        ...
        $branch->delete();
        return redirect()->route('manager.branch.index')->with('m-class', 'success')->with('message', t('Successfully Deleted'));
    }

    public function branches(Request $request)
    {
        $branches = $this->_model->notDraft()->whereHas('merchant')->where('merchant_id', $request->get('id'))->get();
    ...
    public function sendNotification(Request $request)
    {
        ...
        if ($user) {
            event(new BranchNotificationEvent($user, $request->get('title'), $request->get('content')));
